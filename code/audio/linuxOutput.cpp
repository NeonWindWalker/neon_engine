...
void audio_callback(snd_async_handler_t *ahandler);
...
public:
    ...
    ALSAAudioOutput()
    {
        ...
        PlaybackHandle = 0;
        CallbackHandle = 0;
        BUFFERSIZE = AUDIOLIB_PACKET_FRAMES * channels * sizeof(int16) * 4;
        stopFlag = false;
        thread = new Thread(makeDelegate(this, &ALSAAudioOutput::threadFunc), 0);
    }
    ...
    virtual void play(boolean01 b)
    {
        _play = b;
    }

    virtual boolean01 isPlaying()const
    {
        return _play;
    }
    ...
    int audioRecovery(register int err)
    {
    	// Under-run?
    	if (err == -EPIPE)
    	{
    ...
    good:		return(0);
    		logError << "Can't recovery from underrun, prepare failed: " << snd_strerror(err);
    	}
    	...
    	else if (err == -ESTRPIPE)
    	{
    		...
    		logError << "Can't recovery from suspend, prepare failed: " << snd_strerror(err);
    	}

    	return(err);
    }

    void copy_wave_data(const snd_pcm_channel_area_t *buffer, snd_pcm_uframes_t offset, snd_pcm_uframes_t numSamples)
    {
    	...
    	int16* bufPtr = (int16 *)(((unsigned char *)buffer[0].addr) + (offset * 4));

    	Assert(numSamples == AUDIOLIB_PACKET_FRAMES);
    	...
    	for(uint i=0; i < AUDIOLIB_PACKET_FRAMES; i++)
    		for(uint ch=0; ch < channels; ch++)
    			{
    				bufPtr[i * channels + ch] = (int16)(clamp(channelsFrame[ch][i], -1.0f, 1.0f) * 32767.0f);
    			}
    }
    	...
    	while (1)
    	{
    		...
    		switch (snd_pcm_state(PlaybackHandle))
    		{
    			case SND_PCM_STATE_XRUN:
    			{
    				...
    				{
    					logError << "XRUN recovery failed: " << snd_strerror(err);
    out:				return;
    				}
    			...
    			case SND_PCM_STATE_SUSPENDED:
    			{
    				...
    				{
    					logError << "SUSPEND recovery failed: " << snd_strerror(err);
    					goto out;
    				}
    			}
    		}
    		...
    		if ((err = snd_pcm_avail_update(PlaybackHandle)) < 0)
    		{
    			...
    			{
    				logError << "Avail update failed: " << snd_strerror(err);
    				goto out;
    			}
    		...
    		if (err < AUDIOLIB_PACKET_FRAMES)
    		{
    			if (first)
    			{
    				if ((err = snd_pcm_start(PlaybackHandle)) < 0)
    				{
    					logError << "Start error: " << snd_strerror(err);
    					goto out;
    				}
    		...
    		do
    		{
    			...
    			if ((err = snd_pcm_mmap_begin(PlaybackHandle, &buffer, &offset, &frames)) < 0)
    			{
    				...
    				{
    					logError << "MMAP begin error: " << snd_strerror(err);
    					goto out;
    				}
    			...
    			if ((err = snd_pcm_mmap_commit(PlaybackHandle, offset, frames)) < 0 || (snd_pcm_uframes_t)err != frames)
    			{
    				...
    				{
    					logError << "MMAP commit error: " << snd_strerror(err);
    					goto out;
    				}
    ...
    out:				return(err);
    			...
    			if ((err = snd_pcm_mmap_begin(PlaybackHandle, &buffer, &offset, &frames)) < 0)
    			{
    				if ((err = audioRecovery(err)))
    				{
    					logError << "MMAP begin avail error: " << snd_strerror(err);
    					goto out;
    				}
    			}
    ...
    int set_audio_hardware(void)
    {
    	...
    	{
    		logError << "Can't get sound hardware struct " << snd_strerror(err);
    bad1:	snd_pcm_close(PlaybackHandle);
    	...
    	{
    		logError << "Can't init sound hardware struct: " << snd_strerror(err);
    bad2:	snd_pcm_hw_params_free(hw_params);
    	...
    	{
    		logError << "Can't set 16-bit: " << snd_strerror (err);
    		goto bad2;
    	}
    	...
    	if ((err = snd_pcm_hw_params_set_rate(PlaybackHandle, hw_params, AUDIOLIB_FRAME_RATE, 0)) < 0)
    	{
    		logError << "Can't set sample rate: " << snd_strerror(err);
    		goto bad2;
    	}
    	...
    	{
    		logError << "Can't set stereo: %" << snd_strerror(err);
    		goto bad2;
    	}
    	...
    	{
    		logError << "Can't set interleaved playback: " << snd_strerror(err);
    		goto bad2;
    	}
    	...
    	{
    		logError << "Can't set audio buffer size: " << snd_strerror(err);
    		goto bad2;
    	}
    	}
    	...
    	{
    	snd_pcm_uframes_t period_size = AUDIOLIB_PACKET_FRAMES;

    	if ((err = snd_pcm_hw_params_set_period_size_near(PlaybackHandle, hw_params, &period_size, 0)) < 0)
    	{
    		logError << "Can't set audio period: " << snd_strerror(err);
    		goto bad2;
    	}
    	}
    	...
    	{
    		logError << "Can't set hardware params: " << snd_strerror(err);
    		goto bad2;
    	}
    ...
    int set_audio_software(void)
    {
    	...
    	{
    		logError << "Can't get sound software struct: " << snd_strerror(err);
    bad1:	return(err);
    	}
    	...
    	{
    		logError << "Can't init sound software struct: " << snd_strerror(err);
    bad2:	snd_pcm_sw_params_free(sw_params);
    	...
    	if ((err = snd_pcm_sw_params_set_avail_min(PlaybackHandle, sw_params, AUDIOLIB_PACKET_FRAMES)) < 0)
    	{
    		logError << "Can't set audio period size: " << snd_strerror(err);
    		goto bad2;
    	}
    	...
    	// its sound buffer)
    	if ((err = snd_pcm_sw_params_set_start_threshold(PlaybackHandle, sw_params, BUFFERSIZE - AUDIOLIB_PACKET_FRAMES)) < 0)
    	{
    		logError << "Can't set start threshold: " << snd_strerror(err);
    		goto bad2;
    	}
    	...
    	{
    		logError << "Can't set software params: " << snd_strerror(err);
    		goto bad2;
    	}

    	// Tell ALSA to call our audio_callback() function whenever we need to copy more
    	// wave data to the sound card's buffer
    	if ((err = snd_async_add_pcm_handler(&CallbackHandle, PlaybackHandle, AudioLib::audio_callback, this)) < 0)
    	{
    		logError << "Can't register sound callback: " << snd_strerror(err);
    		goto bad2;
    	}
    ...
    void threadFunc(void* args)
    {
		...
		if ((err = snd_pcm_open(&PlaybackHandle, &SoundCardPortName[0], SND_PCM_STREAM_PLAYBACK, 0)) < 0){
			logError << "Can't open audio " << &SoundCardPortName[0] << snd_strerror(err);
			return;
		}
...
void audio_callback(snd_async_handler_t *ahandler)
{
	ALSAAudioOutput* This = (ALSAAudioOutput*)snd_async_handler_get_callback_private(ahandler);
	This->audio_callback(ahandler);
}


IAudioOutput* IAudioOutput::create()
{
